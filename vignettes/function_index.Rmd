...
| Function | Purpose |
|----------|---------|
| `init_spacy_nlp()` | Initialize spaCy with model selection |
| `spacy_parse_full()` | Full parsing with POS, NER, morphology |
| `spacy_extract_entities()` | Extract named entities (span-level) |
| `spacy_extract_noun_chunks()` | Extract noun phrases |
| `spacy_similarity()` | Semantic similarity between texts |
| `spacy_model_info()` | Get loaded model information |

...
Generate draft content grounded in the topic model results rather than AI parametric knowledge. The LLM receives topic labels and term probabilities (beta scores), ensuring outputs are anchored to the input data.
...
3. **Generate drafts**: AI creates content using the top terms ordered by beta scores
...
This "topic-grounded" approach ensures content reflects the actual topic model results, not generic AI knowledge. Prompts include guidelines for person-first language and content-specific best practices.
...
Model how topic prevalence varies with document-level covariates using count regression. Auto-selects between Poisson, Negative Binomial, and Zero-Inflated Negative Binomial based on data diagnostics (dispersion ratio and zero proportion).

...
Topic modeling discovers hidden themes in text collections via STM or
...
STM discovers latent topics while incorporating document metadata as
covariates.

**Best for:** metadata analysis, covariate effects, interpretable
word-probability distributions.

**Quality metrics:** Semantic Coherence, Exclusivity, Held-out
Likelihood.
...
[stm package on CRAN](https://CRAN.R-project.org/package=stm)

## Embedding-based Topics

Uses transformer embeddings with dimensionality reduction and
...
**Backends:** `"python"` (full BERTopic, default) · `"r"` (no BERTopic
needed) · `"auto"`.
...
See: [BERTopic](https://maartengr.github.io/BERTopic/) ·
...
Generate draft content (survey items, research questions, theme
descriptions, policy recommendations, interview questions) anchored to
topic term probabilities rather than generic AI knowledge.
...
The LLM receives topic labels plus terms with beta scores. Use
...
to inspect or override prompts.

## Topic Prevalence Modeling

Model how topic prevalence varies with document covariates. Auto-selects
Poisson, Negative Binomial, or Zero-Inflated NB based on dispersion
...
- **Embedding:** short texts, semantic similarity (`backend = "r"` if no
  BERTopic)
